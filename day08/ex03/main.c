...
int     main(void) {
    ...
    while (42) {
        ...
        val = start_conversion();
        percent = val * 100 / 1023; // getting val in pourcentage
        reset_led(led_frame[0]);
        reset_led(led_frame[1]);
        reset_led(led_frame[2]);
        if (percent >= 33) {
            build_led_frame_but_macros(led_frame[0], 0b00010, BLUE);
        }
        if (percent >= 66) {
            build_led_frame_but_macros(led_frame[1], 0b00001, CYAN);
        }
        if (percent == 100) {
            build_led_frame_but_macros(led_frame[2], 0b00001, WHITE);
        }
