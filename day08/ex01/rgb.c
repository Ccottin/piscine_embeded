...
#define RED (uint8_t[]) { 0xff, 0x00, 0x00 }
...
int     main(void) {
    ...
    uint8_t i = 0;
    uint8_t *color_tab[7] = {RED, BLUE, GREEN, YELLOW, CYAN, MAGENTA, WHITE};
    ...
    while (42) {
        if (i == 7)
            i = 0;
         send_start_frame();
        // set global value to minimal value (see p.5) and red value tu be full red
        build_led_frame_but_macros(led_frame[0], 0b00001, color_tab[i]); // should circle every color
